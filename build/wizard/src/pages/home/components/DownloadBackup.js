...
const Comp = ({ session, fileprefix, path, packageName }) => {

    // const fromPath = path
    // const [fromPath, setFromPath] = React.useState(path);
    ...
    async function downloadFile() {
        try {
            ...
            const fileName = parseFileName(path, blob.type);

            saveAs(blob, fileName);
        } catch (e) {
            console.error(`Error on copyFileFrom ${path}: ${e.stack}`);
        }
    }
